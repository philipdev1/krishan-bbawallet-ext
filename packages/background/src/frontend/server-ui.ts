...
async function handle<T = any>(
  ...
  switch (method) {
    ...
    case UI_RPC_METHOD_BBA_CONNECTION_URL_READ:
      return await handleBbaConnectionUrlRead(ctx);
...
async function handleBbaConnectionUrlRead(
  ctx: Context<Backend>
): Promise<RpcResponse<string>> {
  const resp = await ctx.backend.bbaConnectionUrlRead();
  return [resp];
}
